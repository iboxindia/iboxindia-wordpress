<?php
/**
 * Iboxindia Wordpress Notices
 *
 * Closing notice on click on `ibx-wp-notice-close` class.
 *
 ...
 * `<a href="#" data-repeat-notice-after="<?php echo MONTH_IN_SECONDS; ?>" class="ibx-wp-notice-close">`
...
if ( ! class_exists( 'Iboxindia_WP_Notices' ) ) :

	/**
	 * Iboxindia_WP_Notices
	 *
	 * @since 1.4.0
	 */
	class Iboxindia_WP_Notices {
		...
		public function dismiss_notice() {
			...
			if ( ! apply_filters( 'ibx_wp_notices_user_cap_check', current_user_can( 'manage_options' ) ) ) {
				return;
			}
			...
			if ( false === wp_verify_nonce( $nonce, 'ibx-wp-notices' ) ) {
				wp_send_json_error( esc_html_e( 'WordPress Nonce not validated.', 'iboxindia' ) );
			}
		...
		public function enqueue_scripts() {
			wp_register_script( 'ibx-wp-notices', self::_get_uri() . 'notices.js', array( 'jquery' ), self::$version, true );
			wp_localize_script(
				'ibx-wp-notices',
				'iboxWpNotices',
				array(
		...
		public function show_notices() {
			...
			$defaults = array(
				'id'                         => '',      // Optional, Notice ID. If empty it set `ibx-wp-notices-id-<$array-index>`.
				...
				'display-with-other-notices' => true,    // Should the notice be displayed if other notices  are being displayed from Iboxindia_WP_Notices.
				'is_dismissible'             => true,
			);
		...
		public static function markup( $notice = array() ) {
			...
			wp_enqueue_script( 'ibx-wp-notices' );

			do_action( 'ibx_wp_notice_before_markup' );

			do_action( "ibx_wp_notice_before_markup_{$notice['id']}" );
			...
			<div id="<?php echo esc_attr( $notice['id'] ); ?>" class="<?php echo esc_attr( $notice['classes'] ); ?>" data-repeat-notice-after="<?php echo esc_attr( $notice['repeat-notice-after'] ); ?>">
				<div class="notice-container">
					<?php do_action( "ibx_wp_notice_inside_markup_{$notice['id']}" ); ?>
					<?php echo wp_kses_post( $notice['message'] ); ?>
				</div>
			</div>
			<?php

			do_action( "ibx_wp_notice_after_markup_{$notice['id']}" );

			do_action( 'ibx_wp_notice_after_markup' );
		...
		private static function get_wrap_classes( $notice ) {
			$classes = array( 'ibx-wp-notice', 'notice' );
	 ...
	 */
	Iboxindia_WP_Notices::get_instance();
